...
@interface XQ_TabBarViewController ()<TabBarViewDelegate>
...
- (void)setXQ_centerViewController:(UIViewController *)XQ_centerViewController {
    _XQ_centerViewController = XQ_centerViewController;
...
-(void)tabBarViewCenterItemClick:(UIButton *)button{
    [self presentViewController:_XQ_centerViewController animated:YES completion:nil];
    //    [self.navigationController pushViewController:_xm_centerViewController animated:YES];
}
#pragma mark - 共有方法
-(void)showCenterViewController:(BOOL)show animated:(BOOL)animated{
    
    if (show) {
        [self presentViewController:_XQ_centerViewController animated:animated completion:nil];
        return;
    }
    [self XQTabBarHidden:NO animated:YES];
...
- (void)XQTabBarHidden:(BOOL)hidden animated:(BOOL)animated {
    ...
    [UIView animateWithDuration:duration animations:^{
        if (hidden) {
            self.tabBarView.frame = CGRectMake(0, HEIGHTSCREEN + 50, WIDTHSCREEN, 49);
        }else{
            self.tabBarView.frame = CGRectMake(0, HEIGHTSCREEN - 49, WIDTHSCREEN, 49);
        }
...
-(id)initWithItemSelectedImages:(NSMutableArray *)selected
                   normalImages:(NSMutableArray *)normal
                         titles:(NSMutableArray *)titles{
    if ([super init]) {
        self.frame = CGRectMake(0, HEIGHTSCREEN - 49, WIDTHSCREEN, 49);
...
-(void)createUI{
    ...
    for (int i = 0; i < self.unSelectedImages.count; i ++) {
        ...
        XQButton * button = [XQButton XQ_shareButton];
...
-(void)tabBarBadgeValue:(NSUInteger)value item:(NSInteger)index{
    XQButton * button = self.buttonArray[index];
...
-(void)setItemSelectedIndex:(NSInteger)itemSelectedIndex{
    ...
    for (XQButton * item in self.buttonArray) {
        item.selected = NO;
...
-(void)setTextColor:(UIColor *)textColor{
    _textColor = textColor;
    for (XQButton * button in self.buttonArray) {
        [button setTitleColor:textColor forState:UIControlStateSelected];
    }
}

-(void)buttonClick:(XQButton *)button{
    for (XQButton * item in self.buttonArray) item.selected = NO;
    /**
...
-(void)layoutSubviews{
    ...
    for (int i = 0; i < self.buttonArray.count; i++) {
        XQButton * button = self.buttonArray[i];
